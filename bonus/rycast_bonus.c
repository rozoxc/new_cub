...
/*   Updated: 2025/07/29 02:38:57 by ababdoul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_sky(t_game *game)
{
	...
	while (i < WINDOW_HEIGHT / 2)
	{
		...
		{
			my_mlx_pixel_put(game->image, pj, i, 2201000);
			pj++;
		}
		i++;
	}
...
int	calculate_tex_x(t_ray *ray, t_game *game, double rayDirX, double rayDirY)
{
	int	texX;

	texX = (int)(ray->wallX * (double)game->texture->width);
	if (ray->side == 0 && rayDirX > 0)
		texX = game->texture->width - texX - 1;
...
void	draw_wall_column(t_game *game, t_ray *ray, int x, int texX)
{
	...
	lineHeight = (int)(WINDOW_HEIGHT / ray->perpWallDist);
	step = 1.0 * game->texture->height / lineHeight;
	texPos = (drawStart - WINDOW_HEIGHT / 2 + lineHeight / 2) * step;
	y = drawStart;
	while (y < drawEnd)
	{
		texY = (int)texPos;
		if (texY >= game->texture->height)
			texY = game->texture->height - 1;
		if (texY < 0)
			texY = 0;
		texPos = texPos + step;
		color = get_texture_pixel(game->texture, texX, texY);
		if (ray->side == 1)
...
void	render_3d(t_game *game)
{
	...
	while (x < WINDOW_WIDTH)
	{
		...
		ray = cast_ray(game, rayDirX, rayDirY);
		texX = calculate_tex_x(&ray, game, rayDirX, rayDirY);
		draw_wall_column(game, &ray, x, texX);
		x++;
	}
}
